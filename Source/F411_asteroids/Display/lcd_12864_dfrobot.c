...
{
	uint8_t command = cmd;
	HAL_GPIO_WritePin(LCD_CMD_GPIO_Port, LCD_CMD_Pin, GPIO_PIN_RESET);		//cmd
	HAL_GPIO_WritePin(SPI1_CS_GPIO_Port, SPI1_CS_Pin, GPIO_PIN_RESET);		//select
...
{
	uint8_t dat = data;
	HAL_GPIO_WritePin(LCD_CMD_GPIO_Port, LCD_CMD_Pin, GPIO_PIN_SET);		//data
	HAL_GPIO_WritePin(SPI1_CS_GPIO_Port, SPI1_CS_Pin, GPIO_PIN_RESET);		//select
...
///////////////////////////////////////////////////////////////////////
//Draw icon with image wrapping to other side / top/bottom
//of the LCD
void LCD_DrawIconWrap(uint32_t offsetX, uint32_t offsetY, const ImageData *pImage, uint8_t update)
{
	uint32_t sizeX = pImage->xSize;
	uint32_t sizeY = pImage->ySize;
	uint8_t bitValue = 0x00;
	...
	for (uint32_t i = 0 ; i < sizeY ; i++)
	{
		...
		for (uint32_t j = 0 ; j < sizeX / 8 ; j++)
		{
			...
			while (p > 0)
			{
				...
				if (x < LCD_WIDTH - 1)
					x++;            //increment the x - wrapping enabled
				else
					x = 0;			//wrap
